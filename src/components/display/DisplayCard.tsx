...
const DisplayCard = ({ id, image, name, score, description, releaseDate, tab }:
    {
        id: number,
        image: string,
        name: string,
    ...
    return (
        <Link href={`/${tab}/${id}`} className='flex items-center justify-center h-full w-full bg-primary/40 rounded-md'>
